...
elseif ($_POST['save'] && $module_id)
{
	...
	if (!$strError)
	{
		...
		if (!$strError && $_REQUEST['components'])
		{
			...
			if ($bCustomNameSpace)
			{
				...
				while(false !== $item = readdir($dir))
				{
					...
					{
						$arTmp = BuilderGetFiles($f, array('.svn', '.hg', '.git', '.scss'), true);
				...
				while(false !== $item = readdir($dir)) 
				{
					...
					while(false !== $item0 = readdir($dir0))
					{
						if ($item0 == '.' || $item0 == '..' || !is_dir($f = $path0.'/'.$item0))
							continue;
						$arTmp = BuilderGetFiles($_SERVER['DOCUMENT_ROOT'].'/bitrix/components/'.$item.'/'.$item0, array('.svn', '.hg', '.git', '.scss'), true);
						foreach($arTmp as $file)
							$ar[] = '/'.$item.'/'.$item0.$file;
					}
					closedir($dir0);
		...
		{
			$ar = BuilderGetFiles($m_dir, array('.svn', '.hg', '.git', '.scss'), true);
			...
			{
			/*
				$str = file_get_contents($_SERVER['DOCUMENT_ROOT'].'/bitrix/modules/likee.mpbuilder/samples/_updater.php');
	...
	<tr>
		...
		<td>
			<?
				...
				{
					$updater = file_get_contents($_SERVER['DOCUMENT_ROOT'].'/bitrix/modules/likee.mpbuilder/samples/_updater.php');
