...
if ($_POST['save'])
{
	...
	if (!$strError)
	{
		...
		($str = file_get_contents($f = $_SERVER['DOCUMENT_ROOT'].'/bitrix/modules/likee.mpbuilder/samples/install/index.php')) || die(GetMessage("BITRIX_MPBUILDER_FAYL_NE_NAYDEN").$f);
		...
		($str1 = file_get_contents($f = $_SERVER['DOCUMENT_ROOT'].'/bitrix/modules/likee.mpbuilder/samples/include.php')) || die(GetMessage("BITRIX_MPBUILDER_FAYL_NE_NAYDEN").$f);
	...
	if ($strError)
		...
		LocalRedirect('/bitrix/admin/likee.mpbuilder_step2.php?module_id='.$module.'&lang='.LANGUAGE_ID);
}
?>
