        ...
        if ($info) {
            ...
            if ($ar['type'] == 0 && $info['size'] > 0) // File
            {
                ...
                while (!feof($rs) && ('' !== $str = fread($rs, 512))) {
                    ...
                    if (false && $this->ReadBlockMax && ++$i >= $this->ReadBlockMax) {
                        fclose($rs);
                        return true;
                    }
                }
...
function BuilderGetFiles($path, $arFilter = array(), $bAllFiles = false, $recursive = false)
{
    ...
    if ($dir = opendir($path)) {
        while (false !== $item = readdir($dir)) {
            ...
            if (in_array(substr($item, strrpos($item, '.')), array_merge(array('.svn', '.hg', '.git'), $arFilter)))
                continue;
...
function GetLangPath($file, $m_dir)
{
    ...
    if (preg_match('#^(/install/components/[^/]+)(/[^/]+)(/?.*)$#', $file, $regs)) {
        ...
        elseif (preg_match('#^(/install/components/[^/]+)(/.+)$#', $file, $regs))
            $lang_file = $regs[1] . '/lang/ru' . $regs[2];
    } elseif (preg_match('#^(/install/wizards/[^/]+/[^/]+)(/[^/]+)(/?.*)$#', $file, $regs)) {
        $c_dir = $regs[1];
        $c_path = $regs[2] . $regs[3];

        if (preg_match('#^/site/public#', $c_path)) {
            return false;
        }

        if (preg_match('#^(/site/templates/[^/]+)(/.+)$#', $c_path, $rr)) {
            //echo '<pre>' . print_r($rr, true) . '</pre>';

            if (preg_match('#^(/components/.+)(/[^/]+\.php)$#', $rr[2], $rrr)) {
                //echo '<pre>' . print_r($rrr, true) . '</pre>';

                return $c_dir . $rr[1] . $rrr[1] . $lang . $rrr[2];
            }

            return $c_dir . $rr[1] . $lang . $rr[2];
        }


        return $regs[1] . $lang . $regs[2];
    } else
